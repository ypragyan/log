...
public class DriveIOFalcon500 implements DriveIO {
  ...
  private final TalonFX leftSomething;
  private final TalonFX rightSomething;

  private final Pigeon2 gyro;

  public DriveIOFalcon500() {
    leftLeader = new TalonFX(0);
    rightLeader = new TalonFX(3);
    leftFollower = new TalonFX(1);
    rightFollower = new TalonFX(4);
    leftSomething = new TalonFX(2);
    rightSomething = new TalonFX(5);
    ...
    leftSomething.follow(leftLeader);
    rightSomething.follow(rightLeader);
    
    leftLeader.setInverted(true);
    rightLeader.setInverted(false);
    leftFollower.setInverted(InvertType.FollowMaster);
    rightFollower.setInverted(InvertType.FollowMaster);
    leftSomething.setInverted(InvertType.FollowMaster);
    rightSomething.setInverted(InvertType.FollowMaster);
